...
<head>
  ...
  <title>Kadir Bux Dental Lab & Clinic</title>
  <!-- Tailwind CSS CDN -->
        ...
        <div className="min-h-screen pb-10">
          ...
          <header className="bg-white border-b px-4 py-2 flex items-center justify-between shadow-sm">
            ...
            <div className="flex items-center gap-2 text-xs overflow-x-auto">
              <button className="bg-pink-500 text-white px-3 py-1.5 rounded-full flex items-center gap-1 font-medium whitespace-nowrap">
                <i data-lucide="home" className="w-3.5 h-3.5"></i> Home
              </button>
              <button className="text-gray-600 px-3 py-1.5 flex items-center gap-1 whitespace-nowrap">
                <i data-lucide="users" className="w-3.5 h-3.5"></i> Patients
              </button>
              <button className="text-gray-600 px-2 py-1.5 font-bold">+ New</button>
              <button className="text-gray-600 p-1.5"><i data-lucide="settings" className="w-4 h-4"></i></button>
              <button className="border px-2 py-1 rounded text-purple-600 flex items-center gap-1 whitespace-nowrap">
                <i data-lucide="globe" className="w-3.5 h-3.5"></i> Urdu
              </button>
              <button className="text-gray-500 p-1.5"><i data-lucide="log-out" className="w-4 h-4"></i></button>
            </div>
          </header>
          ...
          <main className="max-w-4xl mx-auto p-4 space-y-6">
            ...
            <div className="text-center py-2">
              ...
              <div className="inline-flex items-center gap-1 text-[11px] bg-emerald-50 text-emerald-600 border border-emerald-200 px-2.5 py-0.5 rounded-full mt-2">
                <i data-lucide="sparkles" className="w-3 h-3"></i> AI assistant ready ✨
              </div>
              ...
              {actionButtons.map((btn, index) => (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-sky-50 flex items-center justify-center text-sky-500">
                    <i data-lucide={btn.icon} className="w-5 h-5"></i>
                  </div>
            ...
            <div className="grid grid-cols-5 gap-2.5">
              {stats.map((stat, index) => (
                <div key={index} className="bg-white p-2.5 rounded-xl border border-gray-100 shadow-sm text-center">
                  <p className="text-[10px] text-gray-400 font-medium mb-1 leading-tight">{stat.label}</p>
